...
def convert_inline(md: str) -> str:
    ...
    for i, code in enumerate(code_spans):
        md = md.replace(
            f"__CODE_{i}__", f'<font face="monospace">{html.escape(code)}</font>'
        )
    return md
...
def _parse_table_block(
    ...
    while idx < len(lines):
        ...
        rest = (
            " | ".join([convert_inline(c) for c in cols[1:]]) if len(cols) > 1 else ""
        )
        bottom = ""
        if len(cols) > 2:
...
def markdown_to_gchat_widgets(md_text: str) -> list[dict[str, Any]]:
    ...
    def flush_para():
        ...
        if all(re.match(r"^\s*[-*•]\s+", l) for l in para_buf if l.strip()):
            ...
            html_text = "<br>".join(
                [convert_inline(l) if l.strip() else "" for l in para_buf]
            )
    ...
    while i < len(lines):
        ...
        if line.strip().startswith("```"):
            ...
            widgets.append(
                {
                    "textParagraph": {
                        "text": f'<font face="monospace">{"<br>".join(code_buf)}</font>'
                    }
                }
            )
            i += 1
