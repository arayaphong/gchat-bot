...
app_dir="$HOME/gchat-bot"

# 1. Cleanup old download
rm -rf "$HOME/development.zip" "$HOME/gchat-bot-development/"

# 2. Download development branch
wget -O "$HOME/development.zip" https://github.com/arayaphong/gchat-bot/archive/refs/heads/development.zip

# 3. Extract
unzip -o "$HOME/development.zip" -d "$HOME/"

# 4. Sync to gchat-bot folder
rsync -av "$HOME/gchat-bot-development/" "$app_dir/"

# 5. Install Python dependencies
# Recreate the venv if it is missing or broken (e.g. moved from another path),
# then install with an absolute interpreter path so this works from any cwd.
